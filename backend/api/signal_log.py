...
def get_signal_log(date: str = None, symbol: str = None):
    ...
    if not all_fut_rows:
        return {"signals": [], "total": 0, "date": today, "snapshots": total_snaps,
                "long_buildup": 0, "short_buildup": 0, "short_covering": 0, "long_unwinding": 0,
                "open_time": to_ist(ts_open), "latest_time": to_ist(ts_latest),
                "message": "No futures data yet — FUT capture started today"}
    ...
    for sym, ts_map in fut_data.items():
        ...
        vol_chg_pct   = round((vol_latest - vol_open) / vol_open * 100, 2)
        ...
        for ts in timestamps:
            ...
            if not snap:
                continue
            s_oi_chg    = (snap["oi"] - oi_open) / oi_open * 100 if oi_open > 0 else 0
            s_price_chg = (snap["last_price"] - price_open) / price_open * 100 if price_open > 0 else 0
            s, _, _ = classify(s_oi_chg, s_price_chg)
            if s == signal_type:
                persistence += 1
                if ts < first_seen_ts:
                    first_seen_ts = ts
        ...
        if cpr:
            ...
            if cmp > tc:   cpr_position = "Above CPR"
            elif cmp < bc: cpr_position = "Below CPR"
            else:          cpr_position = "Inside CPR"
